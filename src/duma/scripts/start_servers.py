...
def main():
    ...
    servers = [
        ("sh scripts/start_duma_server.sh", 8001),
    ]

    # Set up signal handlers
    def signal_handler(signum, frame):
        logger.info("\nReceived shutdown signal, cleaning up...")
        # Only clean up the duma server port since React script handles its own cleanup
    ...
    logger.info("Starting duma server...")
    try:
        ...
        # Cleanup on exit - only clean up duma server
