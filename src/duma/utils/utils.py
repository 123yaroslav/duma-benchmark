...
# Try to get data directory from environment variables first.
# Keep TAU2_DATA_DIR as a legacy fallback for compatibility.
DATA_DIR_ENV = os.getenv("DUMA_DATA_DIR")
LEGACY_DATA_DIR_ENV = os.getenv("TAU2_DATA_DIR")

if DATA_DIR_ENV:
    # Use DUMA_DATA_DIR if set.
    DATA_DIR = Path(DATA_DIR_ENV)
    logger.info(f"Using data directory from environment: {DATA_DIR}")
elif LEGACY_DATA_DIR_ENV:
    # Legacy fallback during migration from tau2 -> duma.
    DATA_DIR = Path(LEGACY_DATA_DIR_ENV)
    logger.warning(
        "TAU2_DATA_DIR is deprecated. Please use DUMA_DATA_DIR instead."
    )
    logger.info(f"Using data directory from legacy environment: {DATA_DIR}")
else:
...
if not DATA_DIR.exists():
    ...
    logger.warning(
        "Set DUMA_DATA_DIR environment variable to point to your data directory"
    )
