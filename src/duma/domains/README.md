...
This folder contains the domains for the duma project.
...
All the data for the domain is stored in `data/duma/domains/<domain_name>` folder.
Should contain:
...
To make it easy for people to use your domain, you need to register your `get_environment` function in duma `registry.py` file.

In `registry.py`:
```python
from duma.your_domain.environment import get_environment as your_domain_get_environment
...
